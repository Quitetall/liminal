...
## F-92 — full fixed-base campaign: execution green, qualification fail-closed

*Recorded on branch `assurance-maintenance` as F-74 on 2026-09-17, by a parallel
session. Renumbered F-92 when the branch was consolidated into main on
2026-09-23: main had meanwhile assigned F-74 to the canary-table defect (the §6
review at `d8006c16`), without seeing the unmerged branch. The branch's commit
messages keep the original number; this ledger uses F-92 so that one number
names one finding.*
