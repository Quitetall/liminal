...
2. For each refused mutant, locate its recorded text in the file and take the
   line number. Where the text now appears more than once, take the occurrence
   nearest the old line.
3. Update BOTH the registry tuple and the packet's `source` field. They are
...
5. `just haq-inventory` until green, then `just ci`.

If an anchor's line no longer exists at all, the mutant needs re-anchoring
rather than renumbering — see F-55, F-59, F-61 and F-64 in
...
`haq.rs` (`generated_case_builders_match_their_recorded_goldens`) fails with the
computed digest as `left` and the recorded one as `right`. Re-record the golden
from `left`, regenerate at 100,000, and commit both.

## 4. The freeze, and what is still open

AM-17.11 (ratified 2026-09-11) froze the qualifier's own implementation —
`haq.rs` and `scripts/haqp_*`. A finding against those is recorded as residual
