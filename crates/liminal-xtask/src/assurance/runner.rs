...
pub(super) fn registered(id: &str, output: &Utf8Path) -> Result<Command> {
    let argv: &[&str] = match id {
        ...
        // M17.5 F-82/F-84/F-90: the suite runs as its two declared halves,
        // never together, each after building the workspace binaries with the
        // tests' own flags. One `cargo nextest run --workspace` put a full ASan
        // build beside the wall-clock crash tests (240s timeouts on tests that
        // take 7.9s alone), and never built liminal-cli's `lim`.
        "nextest" => &["./scripts/run_ci_tests.sh", "portable"],
        "host" => &["./scripts/run_ci_tests.sh", "host"],
        "threaded" => &["just", "test-threaded"],
        "doctest" => &["./scripts/run_ci_tests.sh", "doctest"],
        // F-76: a tracked file over 90 MiB is refused by the remote only at
        // push time, when the blob is already in history.
        "tracked-sizes" => &["./scripts/check_tracked_file_sizes.sh"],
        // F-90: the fuzz lockfile must already describe the sanitizer build.
        "fuzz-lock" => &[
            "cargo",
            "metadata",
            "--manifest-path",
        ...
        // F-79: the host/portable split covers the suite exactly.
        "partition" => &["./scripts/check_ci_partition.sh"],
        "docs" => &["just", "doc"],
        "deny" => &["just", "deny"],
