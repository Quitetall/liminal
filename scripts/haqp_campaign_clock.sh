...
mkdir -p "$(dirname "$OUT")"
printf '{"schema_version":"haqp-campaign-clock-v1","reference_machine":"%s","runs":[{"id":"%s","elapsed_s":%s,"clean":%s,"result":"%s"]}\n' \
  "$(hostname -s)" "$RUN_ID" "$ELAPSED" "$CLEAN" "$([ "$CODE" -eq 0 ] && echo pass || echo fail)" >"$OUT"
exit "$CODE"
